...
export const getUserShortCodes = async(req: Request, res: Response) => {
    const userId = req.userId;
    if(!userId){
        return res.status(401).json({
            message: "Unauthorized"
...
export const deleteUserShortCodes = async(req: Request, res: Response) => {
    const userLinkId = req.params.userLinkId as string;
    const userId = req.userId;
    if(!userId){
        return res.status(401).json({
            message: "Unauthorized"
